...
locals {
  ...
  generic_gpu_instance_sizes = var.enable_gpu ? [
    "n1-standard-1",
    "n1-standard-2",
    "n1-standard-4",
    "n1-standard-8",
  ...
  reserved_sizes = {
    "a100" = var.enable_gpu && var.enable_a100 ? [
      "a2-highgpu-1g",
      "a2-highgpu-2g",
      "a2-highgpu-4g",
    ] : [],
    "l4" = var.enable_gpu && var.enable_l4 ? [
      "g2-standard-4",
      "g2-standard-8",
      "g2-standard-12",
    ] : [],
  ...
  node_pools = concat([
    ...
    },
    {
      ...
      [for size in sizes : merge({
        ...
        disk_size_gb     = 200
        # l4 gpu doesnt support standard
        disk_type          = gpu == "l4" ? "pd-balanced" : "pd-standard"
        image_type         = "COS_CONTAINERD"
        auto_repair        = true
        auto_upgrade       = true
        preemptible        = false
        ...
        # do not include accelerator count for a100 otherwise we hit vCPU limit
    gpu == "a100" ? {} : { accelerator_count = 1 })]]),
  )
